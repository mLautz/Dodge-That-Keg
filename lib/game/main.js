	...
	MainGame = ig.Game.extend({
		...
		setControls: function(){
			...
			if(!ig.ua.mobile){
				...
				this.menuButtons = [
					new ig.TouchButton('continue', ig.system.width/2 - 150, ig.system.height/3 + 30, 300, 40),
					new ig.TouchButton('restart', ig.system.width/2 - 150, ig.system.height/3 + 90, 300, 40)	
				];
			}
		},
			...
			if(ig.ua.mobile){
				this.drawStats = function(){
					if(this.levelExit.level){
						...
						this.statText.draw('Touch here to continue', x, y + 50, ig.Font.ALIGN.CENTER);
						this.statText.draw('Touch here to restart level', x, y + 110, ig.Font.ALIGN.CENTER);
					} else {
						this.statMatte.draw(0,0);
						var x = ig.system.width/2;
