...
.defines(function(){
	MainGame = ig.Game.extend({
		...
		init: function(){
			...
			this.getEntitiesByType(EntityPlayer)[0].animateIn();
		},
		...
		update: function(){
			...
			if(!this.showStats){
				...
				if(ig.input.pressed('continue') && this.levelExit.level){
					this.showStats = false;
					this.levelExit.nextLevel();
					this.parent();
				}
				//should come back and let player restart
				if(ig.input.pressed('restart' && this.levelExit.level)){
					this.showStats = false;
		...
		setControls: function(){
			...
			if(!ig.ua.mobile){
				...
				this.menuButtons = [
					new ig.TouchButton('continue', ig.system.width/2 - 150, ig.system.height/3 + 130, 300, 40),
			...
			if(ig.ua.mobile){
				this.drawStats = function(){
					if(this.levelExit.level){
						...
						this.statText.draw('Touch here to restart level', x, y + 210, ig.Font.ALIGN.CENTER);
					} else {
						this.statMatte.draw(0,0);
						var x = ig.system.width/2;
						...
						var finalTime = '';
						if(this.runTimer.delta()/60 >= 1){
							finalTime += Math.round(this.runTimer.delta()/60) + ' minutes & ';
						}
						finalTime += Math.round(this.runTimer.delta()%60) + ' seconds.';
				...
				this.drawStats = function(){
					if(this.levelExit.level){
						...
						this.statText.draw('Press backspace to restart level.', x, y + 210, ig.Font.ALIGN.CENTER);
					} else {
						this.statMatte.draw(0,0);
						var x = ig.system.width/2;
						var y = ig.system.height/2;
	...
	InstructionScreen = ig.Game.extend({
		...
		init: function(){
			if(!ig.ua.mobile){
				...
				new ig.TouchButton('continue', ig.system.width/2 - 150, ig.system.height/3 + 70, 300, 60);
			}
		},
		...
		draw: function(){
			...
			if(!ig.ua.mobile){
				var x = ig.system.width/2;
				var y = ig.system.height/3;
				this.instructText.draw('Left/Right for movement, X to jump and go up ladders.', x, y, ig.Font.ALIGN.CENTER);
				this.instructText.draw('Press space to begin!', x, y+ 60, ig.Font.ALIGN.CENTER);
			} else {
				var x = ig.system.width/2;
				var y = ig.system.height/3;
				...
				this.instructText.draw('Tap here to begin!', x, y + 100, ig.Font.ALIGN.CENTER);
			}
		},
	});
